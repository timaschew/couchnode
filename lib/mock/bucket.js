...
function _defaultDecode(info) {
  ...
  } else if (info.flags === FLAGS.NF_LIST) {
    return Array.from(info.value);
  } else {
    return new Buffer(info.value);
  }
}
...
MockBucket.prototype._store = function(key, value, options, callback, opType) {
  ...
  this._maybeInvoke(function() {
    ...
    if (opType === 'set') {
    ...
    } else if (opType === 'listAppend') {
      if (!options.hashkey) {
        options.hashkey = key;
      }
      var origPrependList = this.storage.get(key, options.hashkey);
      if (!origPrependList) {
        return callback(new CbError(
            'key does not exist', errs.keyNotFound), null);
      }
      if (origPrependList.lockExpiry && origPrependList.lockExpiry > new Date()) {
        return callback(new CbError(
            'temporary error - key locked', errs.temporaryError), null);
      }

      var origPrependListDecoded = this._decodeDoc(origPrependList);
      if (origPrependListDecoded.length !== undefined) {
        origPrependListDecoded.push(value);
      } else {
        origPrependListDecoded = [value];
      } 
      origPrependList.value = this._encodeDoc(origPrependListDecoded).value;
      origPrependList.cas = _createCas();

      this.storage.set(key, options.hashkey, origPrependList);

      callback(null, origPrependList);

    } else if (opType === 'listPrepend') {
      if (!options.hashkey) {
        options.hashkey = key;
      }
      var origPrependList = this.storage.get(key, options.hashkey);
      ...
      var origPrependListDecoded = this._decodeDoc(origPrependList);
      if (origPrependListDecoded.length !== undefined) {
        origPrependListDecoded.unshift(value);
      } else {
        origPrependListDecoded = [value];
      } 
      origPrependList.value = this._encodeDoc(origPrependListDecoded).value;
...
MockBucket.prototype.listAppend = function(key, value, options, callback) {
  this._store(key, value, options, callback, 'listAppend');
}

MockBucket.prototype.pushBack = function(key, value, options, callback) {
  this.listAppend(key, value, options, callback);
}

MockBucket.prototype.listPrepend = function(key, value, options, callback) {
  this._store(key, value, options, callback, 'listPrepend');
}

MockBucket.prototype.pushFront = function(key, value, options, callback) {
  this.listPrepend(key, value, options, callback);
}

MockBucket.prototype.listGet = function(key, options, callback) {
  callback();
  //  this._store(key, value, options, callback, 'listGet');
}

MockBucket.prototype.listRemove = function(key, index, options, callback) {
  callback();
  // this._store(key, index, options, callback, 'listRemove');  
}

MockBucket.prototype.listSet = function(key, index, value, options, callback) {
  callback();
  // this._store(key, index, value, option, callback, 'listSet');
}

MockBucket.prototype.listSize = function(key, options, callback) {
  callback();
  // this._store(key, options, callback, 'listSize');
}
