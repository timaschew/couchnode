...
MockBucket.prototype._store = function(key, value, options, callback, opType) {
  ...
  this._maybeInvoke(function() {
    ...
    if (opType === 'set') {
      ...
      let bufferList = []
      if (origAppendItem.value.length < 4) {
        bufferList.push(new Buffer('['))
      }  else {
        bufferList.push(origAppendItem.value.slice(0, origAppendItem.value.length - 1))
      }

      if (bufferList[0].length === 1) {
        bufferList.push(encValAppend.value)
        bufferList.push(new Buffer(']'))
      } else {
        bufferList.push(new Buffer(','))
        bufferList.push(encValAppend.value)
      ...
      origAppendItem.value = Buffer.concat(bufferList)
