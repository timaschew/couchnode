...
MockBucket.prototype._store = function(key, value, options, callback, opType) {
  ...
  this._maybeInvoke(function() {
    ...
    if (opType === 'set') {
      ...
      var origAppendItemDecoded = this._decodeDoc(origAppendItem);
      if (origAppendItemDecoded.length !== undefined) {
        origAppendItemDecoded.push(value);
      } else {
        origAppendItemDecoded = [value];
      } 
      origAppendItem.value = this._encodeDoc(origAppendItemDecoded).value;
      origAppendItem.cas = _createCas();

      this.storage.set(key, options.hashkey, origAppendItem);

      callback(null, origAppendItem);
      ...
      var origAppendItemDecoded = this._decodeDoc(origAppendItem);
      if (origAppendItemDecoded.length !== undefined) {
        origAppendItemDecoded.unshift(value);
      } else {
        origAppendItemDecoded = [value];
      } 
      origAppendItem.value = this._encodeDoc(origAppendItemDecoded).value;
